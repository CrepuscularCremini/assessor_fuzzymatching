...
basic_columns = ['geom_id', 'PARCEL_APN','MASTER_PARCEL_APN','TAXAPN','CENSUS_TRACT','CENSUS_BLOCK_GROUP','OWNER_OCCUPIED','USE_CODE_MUNI_DESC','VAL_ASSD_LAND','VAL_ASSD_IMPRV','ASMT_RCDRS_DATE_TRANSFER','AGGR_GROUP','AGGR_LOT_COUNT','ASMT_VAL_TRANSFER','LOT_NUMBER','ZONING','YR_BLT','BUILDING_SQFT','STORIES_NUMBER','VAL_MRKT_LAND','VAL_MRKT_IMPRV','CONSTRUCTION_CODE','FOUNDATION_TYPE','CAL_SQFT','IMPRV_PER_SQFT','VAL_TRANSFER','ASSR_ACREAGE']
basic_columns = [s.lower() for s in basic_columns]
bc = fdf[basic_columns].copy()
bc.to_csv('BasicLightBox/basic_light_box_parcels.csv')
